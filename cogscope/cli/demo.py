"""Cogscope Demo CLI - System-level demonstration of behavioral contract enforcement.

This CLI provides commands to run the system-level demo that shows
Cogscope's value as critical infrastructure for AI systems.
"""
...
app = typer.Typer(
    name="demo",
    help="System-level demo showing Cogscope as behavioral contract enforcement",
)
console = Console()
...
def demo_run(
    ...
    This demonstrates Cogscope's value as an behavioral contract enforcement by showing:

    1. WITHOUT Cogscope: Silent failure - pipeline completes but reasoning
       assumptions are violated, downstream systems execute unsafely.

    2. WITH Cogscope: Explicit blocking - contract violations are caught,
       deployment is blocked, downstream systems are protected.

    Example:
        cogscope demo run                     # Run math scenario comparison
        cogscope demo run -s all              # Run all scenarios
        cogscope demo run -s code -m without  # Show silent failure in code review
    """
    ...
    if output == "rich" and not quiet:
        ...
        console.print(
            Panel(
                ...
                "This demo shows Cogscope protecting AI systems from silent reasoning failures.\n"
                "Real AI systems trust LLM reasoning. When that reasoning degrades,\n"
                "traditional monitoring cannot detect it. Cogscope can.",
    ...
    for s in scenarios:
        ...
        if mode == "compare":
            with Progress(
                ...
                # Determine exit code
                if comparison.with_cogscope.cogscope_blocked:
            ...
            if output == "rich":
                ...
                print(
                    json.dumps(
                        {
                            "scenario": s.name,
                            "without_cogscope": comparison.without_cogscope.to_summary(),
                            "with_cogscope": comparison.with_cogscope.to_summary(),
                            "analysis": {
                ...
                print(f"WITHOUT_COGSCOPE_SAFE: {not comparison.without_cogscope.silent_failure}")
                print(f"WITH_COGSCOPE_BLOCKED: {comparison.with_cogscope.cogscope_blocked}")
            ...
            with Progress(
                ...
                result = run_without_cogscope(s)
                all_results.append(result)

            if output == "rich":
                _print_without_cogscope_result(result, s)
            elif output == "json":
            ...
            with Progress(
                ...
                result = run_with_cogscope(s)
                all_results.append(result)

                if result.cogscope_blocked:
                    exit_code = 1

            if output == "rich":
                _print_with_cogscope_result(result, s)
            elif output == "json":
                print(json.dumps(result.to_summary(), indent=2))
...
def _print_without_cogscope_result(result, scenario):
    """Print result from running WITHOUT Cogscope."""
    ...
    console.print(
        Panel(
            ...
            title="[bold yellow]⚠️ WITHOUT Cogscope[/]",
            border_style="yellow",
        )
    )


def _print_with_cogscope_result(result, scenario):
    """Print result from running WITH Cogscope."""
    if result.cogscope_blocked:
        status_style = "red"
    ...
    console.print(
        Panel(
            f"[bold]Mode:[/] WITH Cogscope\n"
            f"[bold]Completed:[/] {result.pipeline_completed}\n"
            f"[bold]Cogscope Blocked:[/] {result.cogscope_blocked}\n"
            f"[bold]Downstream Would Execute:[/] {result.downstream_would_execute}\n"
            f"[bold]Exit Code:[/] {result.gate_result.exit_code if result.gate_result else 'N/A'}\n"
            f"\n[bold {status_style}]Status: {status_text}[/]"
            f"{violations_text}",
            title=f"[bold green]{icon} WITH Cogscope[/]",
            border_style=border,
        )
    )
...
def list_scenarios() -> None:
    ...
    from cogscope.system_demo.scenarios import get_all_scenarios
...
def explain() -> None:
    ...
    console.print(
        Panel(
            ...
            title="[bold red]Without Cogscope[/]",
            border_style="red",
        )
    )
    ...
    console.print(
        Panel(
            "[bold]THE SOLUTION[/]\n\n"
            "Cogscope acts as a FIREWALL between AI reasoning and downstream systems.\n\n"
            ...
            title="[bold green]With Cogscope[/]",
            border_style="green",
        )
    )

    console.print()
    console.print("[dim]Run 'cogscope demo run' to see this in action.[/]")
...
def quick_demo() -> None:
    ...
    Shows: Problem → Cogscope solution → Blocked deployment
    """
    ...
    console.print()
    console.print("[bold red]Cogscope: AI BEHAVIOR FIREWALL[/]")
